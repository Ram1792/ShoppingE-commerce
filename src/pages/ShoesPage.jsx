...
const ShoesPage = () => {
  ...
  const filteredproduct=selectproduct.length===0? ShoesData :  ShoesData.filter((org)=>selectproduct.includes(org.category)) 
  return (
    <>
    <Navbar/>
     ...
     <div className="pro-select">
      {ShoesData.map((shoes)=>{
        return(
          ...
          <label htmlFor="">
             <input type="checkbox" checked={selectproduct.includes(shoes.category)} onChange={()=>companyhandler(shoes.category)} />
          </label>
          {shoes.category}
       </div>
        )
